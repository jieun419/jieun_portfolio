  ...
  parts: [
    {
      ...
      detail1: [
        'Figma를 활용해 직관적으로 원하는 정보만 보일 수 있도록 구조를 잡고',
        '작업하면서 프로젝트를 구체화했습니다. 현재에도 지속적으로 프로젝트를 구체화하고 있습니다.',
      ],
      detail:
        'Figma를 활용해 직관적으로 원하는 정보만 보일 수 있도록 구조를 잡고, 작업하면서 프로젝트를 구체화했습니다. 현재에도 지속적으로 프로젝트를 구체화하고 있습니다.',
    },
    {
    ...
    {
      title: '라이브러리 사용 없이 무한 스크롤 구현',
      detail1: [
        'React Query를 활용해 구현할 수 있었지만, 라이브러리를 활용하지 않고 직접 구현하고 싶었기에',
        'scrollHeight, scrollTop, clientHeight 요소를 활용해 뷰포트 바닥에 닿을 경우 요청한 데이터만큼만 불러오도록 무한 스크롤을 구현했습니다.',
        '이러한 방식을 통해 초기 로딩 시 모든 데이터를 불러오지 않고 사용자가 스크롤 할 때 필요한 만큼의 데이터를 불러와 성능을 향상시키고 사용자 경험을 개선할 수 있었습니다.',
        '이후에는 React Query에서 제공하는 useInfiniteQuery 쿼리를 사용해 구현할 예정입니다.',
      ],
      detail:
        'React Query를 활용해 구현할 수 있었지만, 라이브러리를 활용하지 않고 직접 구현하고 싶었기에 scrollHeight, scrollTop, clientHeight 요소를 활용해 뷰포트 바닥에 닿을 경우 요청한 데이터만큼만 불러오도록 무한 스크롤을 구현했습니다.\n\n이러한 방식을 통해 초기 로딩 시 모든 데이터를 불러오지 않고 사용자가 스크롤 할 때 필요한 만큼의 데이터를 불러와 성능을 향상시키고 사용자 경험을 개선할 수 있었습니다.\n\n이후에는 React Query에서 제공하는 useInfiniteQuery 쿼리를 사용해 구현할 예정입니다.',
    },
    {
      title: '원하는 포켓몬을 쉽게 찾기 위해 검색 페이지 및 검색 기능 구현',
      detail1: [
        ...
        '또한, 원하는 검색 결과가 없을 경우 조건문을 이용하여 특정 화면을 보이도록해 사용자 경험을 향상시켰습니다.',
        '이후에는 React Query에서 제공하는 useInfiniteQuery 쿼리를 사용해 구현할 예정입니다.',
      ],
      detail:
        'input의 action 속성을 통해 페이지가 이동하면서 URL 파라미터에 검색 키워드가 저장됩니다.\n\nURLSearchParams 메서드를 활용해 검색 키워드를 추출하고 React Query의 useQuery와 Redux Toolkit을 활용해 filtering된 검색 결과를 표시했습니다.\n\n또한, 원하는 검색 결과가 없을 경우 조건문을 이용하여 특정 화면을 보이도록해 사용자 경험을 향상시켰습니다.',
  ...
  trouble: [
    {
      ...
      detail1: [
        'useEffect를 활용해 초기 로딩시 포켓몬 데이터 리스트를 업로드하는 과정에서 3번의 호출이 감지 되었습니다.',
        '초반에는 무한 스크롤로 인해 뷰포트가 바닥을 감지하여 이루어지는 이슈로 생각해 스켈레톤을 추가했으나, 동일한 문제가 발생했습니다.',
    ...
    },
    {
      ...
      detail1: [
        ...
        '두 번째로, React.lazy와 Suspense를 활용해 코드 스플리팅을 도입하여 초기 로딩 시의 성능을 향상시켰습니다.',
        '마지막으로, 이미지의 로딩 최적화 하기위해 img태그에 loading="lazy" 속성을 추가했습니다.',
        '이를 통해 뷰포트에 보이지 않는 이미지 로딩 속도를 늦춰 성능 향상을 이끌었습니다.',
      ],
      detail:
