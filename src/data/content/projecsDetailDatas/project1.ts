export const project1 = {
  ...
  parts: [
    ...
    },
    {
      ...
      detailList: [
        `클라이언트의 요구사항을 시각화 해 같이 전달하면서 요구사항을 명확하게 전달하고, 주기적인 회의를 통해 개선해 나감`,
      ],
    },
    {
      title: '기획이 필요한 페이지의 경우 기획 역할 수행',
      detailList: [
        `클라이언트가 기획하지 못한 페이지를 기획하고 이를 문서화`,
        `클라이언트, 프론트엔드 개발자와 소통하며 프로젝트 방향성을 구체화`,
      ],
    },
    {
      title: '전박적이 웹 디자인 (PC, Mobile)',
      detailList: [`클라이언트가 요구한 로고, 색상 코드, 컨셉에 맞춰 디자인 작업 수행`],
    },
    {
      title: '레거시(table 태그) 코드 개선을 통한 유지보수성 향상',
      detailList: [
        `**과거에 활용되었던 템플릿 기반**으로 진행 중인 상황으로 리팩토링 작업이 필요한 상황`,
        `기존 레거시 코드였던 \`table\` 태그 등을 게워내고, \`Semantic Tag\`를 사용하며 개선하는 작업을 수행`,
        `코드의 가독성을 높히고 유지보수성을 향상`,
      ],
    },
    {
      title: 'QA 진행을 통한 프로젝트 품질 향상',
      detailList: [
        `\`QA\`업무에서 기능이 올바르게 작동하고 화면이 요구사항에 따라 표시 되는지 확인하며 발견된 개선 사항 등을 문서화 작업`,
        `이를 프론트엔드 개발자와 공유하며 프로젝트의 품질을 지속적으로 향상`,
      ],
    },
    {
      ...
      detailList: [
        ...
        `요구사항에 따라 \`slick slider\`와 같은 외부 라이브러리를 활용해 동적인 화면을 보여줌`,
        `간단한 토글 버튼과 같은 작업에는 \`jQuery\`를 사용해 구현`,
      ],
    },
  ],
};
