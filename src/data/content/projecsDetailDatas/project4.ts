  ...
  parts: [
    {
      ...
      detailList: [`팀원들과 함께 오버플로우 홈페이지의 **UX를 분석**하고 짧은 작업 기간을 고려해 기술들을 레벨 나눔`],
    },
    {
      title: '공통 컴포넌트 작업',
    },
    ...
    },
    {
      ...
      detailList: [
        `질문 작성 / 수정하기 페이지 작업을 **2인 1조**로 작업 구역을 나눠 기능 구현을 진행했으며, 도움이 필요한 경우 협업하며 진행`,
        `질문 작성란의 경우 사용자에게 더 많은 컨텐츠를 제공하기위해 textarea 요소를 사용하지 않고 \`react-quill\` 라이브러리를 활용하여 구현`,
      ],
    },
    {
      title: '질문 작성 / 수정하기 태그 3개 추가, 수정할 수 있도록 작업',
      detailList: [
        `질문 작성 / 수정하기 페이지에서 태그 영역을 맡음`,
        `태그는 최대 3개 까지만 추가 가능하며, 3개 초과 시 다른 태그를 선택할 수 없도록 select box의 \`disabled\` 속성과 \`some\` 메서드 활용하여 구현`,
      ],
    },
  ],
  trouble: [
    {
      ...
      detailList: [
        `**[문제점]** 질문 작성 / 수정 페이지의 태그 영역 작업 중, 백엔드에서 태그 데이터 완성이 지연되었습니다. 페이지 개발 일정은 촉박했고, 계속 백엔드 데이터 완성을 기다릴 수 없는 상황이었습니다.`,
        `**[해결]** 회의를 통해 구성된 데이터 구조를 파악하고, 이를 기반으로 **실제 데이터와 유사한 \`더미 데이터\`** 를 만들어 작업을 진행했습니다. 백엔드에서 데이터가 완성 이후 더미 데이터 영역만 백엔드 데이터로 교체하여 최종 페이지를 완성했습니다.`,
      ],
    },
    {
      title: 'HTTP로 서버 통신하는 상태로 배포 진행하기',
      detailList: [
        `**[문제점]** 비용 문제로 \`HTTP\`로 서버와 통신하고 중, Github과 Vercel에서 배포 시도해 보았지만 \`HTTPS\`로 배포해야 한다는 이슈로 배포를 할 수 없던 상황이였습니다.`,
        `**[해결]** AWS의 경우 비용 발생으로 인해 고민하던 중, \`Netlify\`로 배포시 \`proxy\`를 활용해 문제를 해결할 수 있는 방안을 찾게 되었습니다. 최상단 경로에 netlify.toml를 생성해 경로를 설정한 후, vite.config에 서버 경로에 대한 작성을 통해 문제를 성공적으로 해결할 수 있었습니다.`,
      ],
    },
  ],
};
